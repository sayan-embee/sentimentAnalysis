...
BEGIN
 ...
 --       SELECT @DuplicateTicketIds = CONCAT(@DuplicateTicketIds,',',CaseNumber) FROM CTE
	...
	FROM @udt_TicketDetailsList udt 
	WHERE udt.AssignedToEmail NOT IN (SELECT C.UserEmail FROM dbo.[T_Conversations] C WITH(NOLOCK) WHERE C.Active = 1 AND C.UserName = 'UserApp')
